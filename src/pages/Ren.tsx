...
import { Cpu, Share2, Download } from "lucide-react";
...
export default function Ren() {
  ...
  const createConversation = async () => {
    ...
    const { data } = await supabase
      .from("conversations")
      .insert({ user_id: user.id, title: "New Ren Chat", mode: "ren" })
      .select()
      .single();
    if (data) {
  ...
  const handleShareChat = () => {
    ...
    const text = messages.map(m => `${m.role === "user" ? "You" : "Ren"}: ${m.content}`).join("\n\n");
  ...
  const handleDownloadChat = () => {
    ...
    a.href = url;
    a.download = `ren-2.5-${new Date().toISOString().slice(0, 10)}.txt`;
    a.click();
  ...
  return (
    <div className="relative flex h-screen overflow-hidden bg-background">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,hsl(210_90%_55%/0.10),transparent_60%)]" />
      <ChatSidebar
      ...
      <div className="relative z-10 flex flex-1 flex-col">
        <header className="flex items-center gap-3 border-b border-border bg-background/80 px-4 py-3 backdrop-blur-sm">
          ...
          />
          <Cpu className="h-5 w-5" style={{ color: "hsl(210 90% 55%)" }} />
          <h2 className="font-display text-lg" style={{ color: "hsl(210 90% 45%)" }}>Ren 2.5</h2>
          <span
            className="rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider"
            style={{
              borderColor: "hsl(210 90% 55% / 0.4)",
              backgroundColor: "hsl(210 90% 55% / 0.1)",
              color: "hsl(210 90% 45%)",
            }}
          >
            GLM
        ...
        {!activeConvoId && messages.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8">
            <div
              ...
              style={{
                backgroundColor: "hsl(210 90% 55% / 0.12)",
                boxShadow: "0 0 0 1px hsl(210 90% 55% / 0.4)",
              }}
            >
              <Cpu className="h-10 w-10" style={{ color: "hsl(210 90% 55%)" }} />
            </div>
            <h2 className="font-display text-2xl" style={{ color: "hsl(210 90% 45%)" }}>
              Ren 2.5 — General Language Model
            </h2>
            <p className="max-w-md text-center text-muted-foreground">
              Precise, structured, and reliable. Ask about code, math, science, writing, analysis — anything.
            </p>
            <Button
              onClick={createConversation}
              className="gap-2 text-white"
              style={{ backgroundColor: "hsl(210 90% 50%)" }}
            >
              Start Ren Chat
            </Button>
          </div>
