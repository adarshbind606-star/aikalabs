...
import { Plus, MessageSquare, Trash2, LogOut, Cherry, ImagePlus, Settings, Flame, Crown, Puzzle } from "lucide-react";
...
interface ChatSidebarProps {
  ...
  activePage?: "chat" | "image" | "settings" | "unbound" | "comet" | "plugins";
}
  ...
  return (
    <>
      ...
      >
        ...
        <div className="border-b border-sidebar-border p-2">
          <Button
            onClick={() => { navigate("/plugins"); onClose(); }}
            className={cn(
              "w-full gap-2 border-fuchsia-400/40 text-fuchsia-300 hover:bg-fuchsia-400/10 hover:text-fuchsia-200",
              activePage === "plugins" && "bg-fuchsia-400/15 text-fuchsia-200"
            )}
            variant="outline"
            size="sm"
          >
            <Puzzle className="h-4 w-4 shrink-0" />
            <span className="truncate text-xs font-semibold tracking-wide">Plugins</span>
          </Button>
        </div>
